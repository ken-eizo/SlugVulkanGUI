...
// Load a glyph by native FreeType glyph index while storing it under an explicit Slughorn
// numeric key. This is the bridge needed by shaping engines such as HarfBuzz: GSUB may return
// glyphs that have no one-to-one Unicode codepoint (ligatures/contextual forms), but the Slug
// atlas still needs an immutable outline/metrics record for the resulting glyph.
bool loadGlyphIndex(
	FT_Face face,
	uint32_t glyphIndex,
	uint32_t keyCodepoint,
	Atlas& atlas,
	LoadConfig* config=nullptr
);
...
bool loadGlyphIndex(
	...
	if(!face || glyphIndex >= static_cast<uint32_t>(face->num_glyphs)) return false;
	if(atlas.hasKey(Key(keyCodepoint, cfg.mask))) return true; // already present - not an error
	if(FT_Load_Glyph(face, static_cast<FT_UInt>(glyphIndex), FT_LOAD_NO_SCALE)) return false;
	...
	atlas.addShape(Key(keyCodepoint, cfg.mask), data);
	return true;
}
...
bool loadGlyph(
	...
	return loadGlyphIndex(face, glyphIndex, codepoint, atlas, config);
}
