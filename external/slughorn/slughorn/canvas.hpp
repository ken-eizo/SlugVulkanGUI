...
public:
	...
	bool defineShape(Key key, slug_t scale=1_cv, Atlas::ShapeInfo::Origin origin={}, bool evenOdd=false) {
		_consolidate();

		return _commitShape(_path._pendingCurves, key, scale, origin, _path.pendingContourStarts(), evenOdd);
	}
	...
	bool defineShape(const Path& p, Key key, slug_t scale=1_cv, Atlas::ShapeInfo::Origin origin={}, bool evenOdd=false) {
		if(!p.hasPendingPath()) return false;

		return _commitShape(_merged(p), key, scale, origin, p.mergedContourStarts(), evenOdd);
	}
