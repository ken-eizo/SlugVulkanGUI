...
struct Sampler {
	...
	Sample renderSample(slug_t rx, slug_t ry, slug_t ppeX, slug_t ppeY) const {
		...
		out.fill = _calcCoverage(out.xcov, out.ycov, out.xwgt, out.ywgt, shape.evenOdd);
...
private:
	...
	static slug_t _calcCoverage(slug_t xcov, slug_t ycov, slug_t xwgt, slug_t ywgt, bool evenOdd) {
		...
		slug_t coverage = std::max(weighted, conservative);

		if(!evenOdd) return _clamp(coverage, 0_cv, 1_cv);

		// Eric Lengyel's reference shader preserves the signed winding accumulation and folds the
		// final positive coverage into alternating [0,1] intervals for the even-odd rule.
		const slug_t half = coverage * 0.5_cv;
		const slug_t phase = half - std::floor(half);
		return 1_cv - std::abs(1_cv - phase * 2_cv);
	}
