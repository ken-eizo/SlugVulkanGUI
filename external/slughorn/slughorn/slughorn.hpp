...
public:
	...
	struct ShapeInfo {
		...
		// Preserve the authored fill rule all the way to the renderer. The Slug reference
		// shader carries this as the E flag in the packed glyph metadata instead of rewriting
		// contours on the CPU, so quadratic winding information remains exact.
		bool evenOdd = false;
	...
	struct Shape {
		...
		// Authored fill rule. Vulkan/Direct3D backends can pack this into the Slug reference
		// shader's E flag (0x1000 in glyphData.w).
		bool evenOdd = false;
...
inline std::ostream& operator<<(std::ostream& os, const Atlas::ShapeInfo& info) {
	return os
		...
		<< " fillRule=" << (info.evenOdd ? "evenodd" : "nonzero")
