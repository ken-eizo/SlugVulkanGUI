...
// WHAT IS SUPPORTED
// -----------------
// - fill-rule="evenodd": preserved as authored shape metadata and evaluated by the Slug
//   coverage rule at render time (the same E-flag model used by Eric Lengyel's reference shader)
//
...
std::pair<Atlas::ShapeInfo, Transform> decomposePath(const NSVGshape* shape, slug_t scale, Atlas::ShapeInfo::Origin origin, bool autoMetrics) {
	...
	// Restore authoring order for stable contour access/diagnostics. Fill-rule evaluation itself
	// is order-independent and remains exact in the Slug coverage stage.
